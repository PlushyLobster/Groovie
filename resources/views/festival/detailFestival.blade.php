...
@section ('content')
    <main id="detailFest-main">
            ...
            <div id="detailFest-infos">
                <div id="detailFest-bloc1">
                    <div id="detailFest-nameDate">
                        <h1>{{ $festival->name }}</h1>
                        <p>Du {{ Carbon::parse($festival->start_datetime)->translatedFormat('j F Y') }}
                            au {{ Carbon::parse($festival->end_datetime)->translatedFormat('j F Y') }}</p>
                    </div>
                ...
                <div id="detailFest-bloc2">
                    ...
                    <div id="detailFest-progText">
                        <select id="date-select" class="mb-4 p-2 border rounded">
                        ...
                        @foreach($uniqueDates as $date)
                            <div class="program-details" data-date="{{ Carbon::parse($date)->format('Y-m-d') }}" style="display: none;">
                                <div class="mb-6">
                                    <p class="text-lg font-semibold">{{ Carbon::parse($date)->translatedFormat('j F Y') }}</p>
                                    @foreach(collect($programmation)->where('day_presence', $date) as $program)
                                    ...
                                    @foreach(collect($programmation)->where('day_presence', $date) as $program)
                                        @foreach($program['artists'] as $artist)
                                            <div class="flex items-center justify-between bg-gray-100 p-4 rounded-lg shadow-md mb-2">
                                                ...
                                                <span class="text-gray-600">{{ Carbon::parse($program['start_time'])->format('H\hi') }}</span>
                                            </div>
                                        @endforeach
                                    @endforeach
                                </div>
        ...
        @if(!\Illuminate\Support\Facades\Auth::check() || $festival->tickets->where('Id_user', \Illuminate\Support\Facades\Auth::id())->count() == 0)
                ...
                <div id="validTicket-formDiv">
                    ...
                    <form id="validTicket-form" method="POST">
                        ...
                        <div class="dateScan-div">
                            <div class="validBillet-div">
                                <input type="date" id="validTicket-date" name="validTicket-date" placeholder="Date" required>
        ...
        @else
            <div id="detailFest-right" style="display: flex; justify-content: center; align-items: center; height: 100%;">
                <div style="text-align: center;">
                    <p style="color: white;">Vous possédez déjà un billet pour ce festival. Vous pouvez aller sélectionner un trajet.</p>
                    <form action="{{ route('trajet') }}" method="GET">
                        <button type="submit" class="validBilletBtn">
                            Aller à la page des trajets
                            <span>
                        <svg width="19" height="15" viewBox="0 0 19 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M2 7.09985L7.625 12.5999L17 1.59985" stroke="#9747FF" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function showProgramDetails(date) {
                programDetails.forEach(detail => {
                    if (detail.getAttribute('data-date') === date) {
            ...
            dateSelect.addEventListener('change', function() {
                showProgramDetails(this.value);
            });

            // Show the first date's program details by default
            if (dateSelect.value) {
                showProgramDetails(dateSelect.value);
            }
        });
    </script>
@endsection
